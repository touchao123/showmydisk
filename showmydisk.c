...
bool should_include_mountpoint(const char *mountpoint, char *include_filters[], int include_count, char *exclude_filters[], int exclude_count, char *type_include_filters[], int type_include_count, char *type_exclude_filters[], int type_exclude_count,const char* filesystem_type);
...
int main(int argc, char *argv[]) {
	...
	for (i = 1; i < argc; i++) {
		...
		} else if (strcmp(argv[i], "--help") == 0) { //添加帮助信息
	...
	while ((mnt = getmntent(fp)) != NULL) {
		...
		// 应用过滤器
		if (!should_include_mountpoint(mnt->mnt_dir, include_filters, include_count, exclude_filters, exclude_count, type_include_filters, type_include_count, type_exclude_filters, type_exclude_count, mnt->mnt_type)) {
			continue;
		}
